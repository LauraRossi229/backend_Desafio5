    ...
    try {
        ...
        if (user) {
            if (user.password == password) {
                ...
                res.redirect(`/products?first_name=${userData.first_name}&last_name=${userData.last_name}&email=${userData.email}`);
            } else {
                //res.status(401).send({ resultado: 'Contaseña no valida', message: password })
                res.status(401).render('login', { errorMessage: 'Contraseña no válida' });
            }
        } else {
            //res.status(404).send({ resultado: 'Not Found', message: user })
            res.status(404).render('login', { errorMessage: 'Usuario no encontrado' });
        }

    } catch (error) {
        //res.status(400).send({ error: `Error en Login: ${error}` })
        res.status(400).render('login', { errorMessage: `Error en el inicio de sesión: ${error}` });
    }
})
